// FitTogether V2.0.51: delete button on every training-plan card and cleaner card controls.
const FT251='V2.0.51'
...
const deletePlan251=(index,name)=>{
...
const enhancePlans251=()=>{
  ...
  wraps.forEach((wrap,index)=>{
    ...
    // Custom plans already receive the V2.0.50 delete control; add one only when missing.
    if(card.querySelector('.plan-delete-v250')||wrap.querySelector('.plan-delete-v251'))return
    const del=document.createElement('button')
    del.type='button'
    del.className='plan-delete-v251'
    del.textContent='×'
    del.setAttribute('aria-label','Plan löschen')
    del.onclick=e=>{e.preventDefault();e.stopPropagation();deletePlan251(index,plan.name)}
...
const version251=()=>document.querySelectorAll('body *').forEach(el=>{
  ...
  if(t.includes('V2.0.50'))el.textContent=t.replaceAll('V2.0.50',FT251)
})
let q251=false
