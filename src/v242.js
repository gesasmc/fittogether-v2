// FitTogether V2.0.42: simple complete free-workout flow with ordering.
export const FITTOGETHER_VERSION='V2.0.42'
...
const move242=(index,dir)=>{
  const list=read242(KEY242,[]);const next=index+dir
  if(next<0||next>=list.length)return
  ;[list[index],list[next]]=[list[next],list[index]]
  write242(KEY242,list);renderFree242();window.FitTogetherCloud?.upload?.()
}
...
const renderFree242=()=>{
  ...
  const items=read242(KEY242,[]);if(!items.length)return
  ...
  box.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{const list=read242(KEY242,[]);list.splice(Number(b.dataset.remove),1);write242(KEY242,list);renderFree242();window.FitTogetherCloud?.upload?.()})
  box.querySelector('.free-start-all-v242').onclick=()=>startWorkout242(read242(KEY242,[]))
}

let queued242=false
const enhance242=()=>{queued242=false;renderFree242();document.querySelectorAll('body *').forEach(el=>{if(el.children.length)return;const t=el.textContent||'';if(t.includes('V2.0.41'))el.textContent=t.replaceAll('V2.0.41',FITTOGETHER_VERSION)})}
const schedule242=()=>{if(queued242)return;queued242=true;requestAnimationFrame(enhance242)}
if(typeof document!=='undefined'){
 const observer=new MutationObserver(m=>{if(m.some(x=>x.addedNodes.length||x.removedNodes.length))schedule242()})
