...
const render305=(force=false)=>{
  ...
  const profile=read305('ft-profile',{}),weights=read305('ft-weight-history',[]),current=num305(weights.at(-1)?.weight),height=num305(profile.height),goalData=read305(goalKey305,null)||{},goal=num305(goalData?.weight),goalStart=num305(goalData?.weightAtSet),bmi=bmi305(current,height),range=refRange305(height),signature=[current,height,goal,goalStart,profile.goal,weights.length].join('|')
  ...
  const diff=goal&&current?goal-current:0,start=goalStart||current,totalDistance=goal&&start?Math.abs(goal-start):0,progress=goal&&current?(totalDistance<.05?100:Math.max(0,Math.min(100,(1-Math.abs(goal-current)/totalDistance)*100))):0
  const goalText=!goal?'Noch kein Ziel gesetzt':Math.abs(diff)<.05?'Ziel erreicht':`${fmt305(Math.abs(diff))} kg bis zum Ziel`
