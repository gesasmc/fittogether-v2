...
let q254=false;const schedule254=()=>{if(document.querySelector('.rest-overlay')||q254)return;q254=true;requestAnimationFrame(()=>{q254=false;enhance254()})}
if(typeof document!=='undefined'){document.addEventListener('click',click254,true);const obs=new MutationObserver(m=>{if(document.querySelector('.rest-overlay'))return;if(m.some(x=>x.addedNodes.length||x.removedNodes.length))schedule254()});const start=()=>{enhance254();obs.observe(document.body,{childList:true,subtree:true})};document.body?start():document.addEventListener('DOMContentLoaded',start,{once:true})}
