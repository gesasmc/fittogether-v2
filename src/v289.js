// FitTogether V2.0.115: compact resume card without DOM churn while training is minimized.
...
const render289=()=>{
  if(document.querySelector('.active-training,.rest-overlay'))return
  const state=state289(),welcome=document.querySelector('.welcome'),home=welcome?.parentElement||document.querySelector('main');let card=document.querySelector('.resume-training-v289')
  if(!state?.exercises?.length||!home){card?.remove();return}
  const i=Math.min(state.exercises.length,Math.max(1,(Number(state.index)||0)+1)),set=Math.max(1,Number(state.set)||1),sets=Math.max(1,Number(state.exercises?.[i-1]?.sets)||3),name=state.exercises?.[i-1]?.name||'Training',sig=[name,i,state.exercises.length,set,sets,Number(state.startedAt)||0].join('|')
  if(!card){card=document.createElement('section');card.className='resume-training-v289';welcome?.insertAdjacentElement('afterend',card)||home.prepend(card)}
...
if(typeof document!=='undefined'){
  const relevant289=node=>node?.nodeType===1&&!node.closest?.('.resume-training-v289')&&!node.classList?.contains('resume-training-v289')
  const start=()=>{render289();new MutationObserver(m=>{if(m.some(x=>[...x.addedNodes,...x.removedNodes].some(relevant289)))schedule289()}).observe(document.body,{childList:true,subtree:true});window.addEventListener('pageshow',schedule289);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')schedule289()});setInterval(()=>{if(document.querySelector('.resume-training-v289'))render289()},30000)}
