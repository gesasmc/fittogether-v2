...
const openEditor234=index=>{
  ...
  const modal=document.createElement('div');modal.className='history-modal-v234';const cardio=x.kind==='cardio'||x.actualSeconds!=null
...
const refreshHistory234=(force=false)=>{
  ...
  const rows=read234(KEY234,[]);const signature=JSON.stringify(rows.map(x=>[x.date,x.name,x.planTitle,x.kind,x.actualSeconds,x.durationSeconds,x.exercises]))
