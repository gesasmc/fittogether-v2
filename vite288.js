export default function trainingResume288(){
  return {
    ...
    transform(code,id){
      ...
      const persist="useEffect(()=>{if(typeof window!=='undefined')window.__FT_RESUME_289=false;if(!resumeWanted289){try{localStorage.removeItem('ft-active-training-v288');localStorage.removeItem('ft-rest-deadline-v289')}catch{}}},[]);useEffect(()=>{const tick=()=>setElapsed288(Math.max(0,Math.floor((Date.now()-startedAt288)/1000)));tick();const id=setInterval(tick,1000);const vis=()=>tick();document.addEventListener('visibilitychange',vis);window.addEventListener('pageshow',vis);return()=>{clearInterval(id);document.removeEventListener('visibilitychange',vis);window.removeEventListener('pageshow',vis)}},[startedAt288]);useEffect(()=>{writeStore('ft-active-training-v288',{exercises,index,set,weight,reps,tip,resting,startedAt:startedAt288,updatedAt:Date.now()})},[exercises,index,set,weight,reps,tip,resting,startedAt288]);const fmtElapsed288=v=>{const h=Math.floor(v/3600),m=Math.floor(v%3600/60),s=v%60;return h>0?String(h)+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0'):String(m)+':'+String(s).padStart(2,'0')};"
